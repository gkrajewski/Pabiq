...
data <- read.csv("PABIQIT_SW_PL_20231024.csv")

#Wybierz plik z kluczami:
keys <- read.csv("PABIQ-labels-IT.csv")
keys <- keys %>% filter(keys$Label != "")
keys <- keys %>% filter(keys$If.needed != "")

#Wybierz plik z tłumaczeniami
trans <- read.csv("PABIQ-translations-IT.csv")
...
write.csv(data_fin,"data_ready_it_pl.csv",fileEncoding = "UTF-8")
