...
#CHOOSE DATA
data_calc <- read.csv("data_ready_it_pl.csv")

#AGE CALCULATION
data_calc$age_months <- age_months(data_calc$date_birth, data_calc$sb_date)
...
data_calc$problem_school <-ifelse(data_calc$problem_school == "", NA, ifelse(data_calc$problem_school == 0, 0, str_count(data_calc$problem_school,",")+1))

data_calc$family_difficulties <- (20 - data_calc$problem_read_write - data_calc$problem_understand - data_calc$problem_speak - data_calc$problem_school)/2
...
write.csv2(data_calc_exp,"data_calc_it_pl.csv",fileEncoding = "UTF-8")
